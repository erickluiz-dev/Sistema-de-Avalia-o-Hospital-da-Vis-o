...
import { Angry, Frown, Meh, Smile, Laugh, Eye, EyeOff } from 'lucide-react'
import {
...
function HomeScreen({ onNav, onLogout, usuario }: { onNav: (s: Screen) => void; onLogout: () => void;  usuario: Usuario | null }) {
  const cards = [
    {
      icon: (
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <NavBar onLogout={onLogout} subtitle="Menu Inicial" usuario={usuario}/>
...
function DashboardScreen({ onBack, onLogout, usuario }: { onBack: () => void; onLogout: () => void;  usuario: Usuario | null }) {
...
export default function App() {
  ...
  return (
    <>
      ...
      {screen === 'dashboard' && <DashboardScreen onBack={() => setScreen('home')} onLogout={() => setScreen('login')} usuario={usuario}/>}
    </>
  )
}
